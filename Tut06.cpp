...
int main(int argc, char* argv[]) {
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    // light parameters
    float phi = glfwGetTime() * 0.5f * 3.14159265f;
    vec3 lightPosition = vec3(0.6f, 50.0f, 5.0f); // fixed position
    // If you want a moving light, uncomment below:
    // vec3 lightPosition = vec3(cosf(phi) * cosf(phi), sinf(phi), -cosf(phi) * sinf(phi)) * 5.0f;
    ...
    mat4 lightProjMatrix = glm::perspective(glm::radians(50.0f), (float)SHADOW_WIDTH / (float)SHADOW_HEIGHT, lightNearPlane, lightFarPlane);
    ...
    // c- bind depth map framebuffer to output the depth values
    {
      ...
      // Bind depth map framebuffer as output
      glBindFramebuffer(GL_FRAMEBUFFER, depthMapFBO);
    ...
    // Everything below here is the moving camera exercise
